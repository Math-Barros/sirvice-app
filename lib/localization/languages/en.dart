const EN_LANG = {
  // Onboarding Page
  'onboarding_page1_title': 'Welcome to SIRVICE!',
  'onboarding_page1_body':
      'Welcome to our community of buying and selling freelancers, we hope you thrive! Please read through this guide to get a kick start',
  'onboarding_page2_title': 'How to add a deal',
  'onboarding_page2_body':
      'Select the freelancer you want, press the ＋ button to add a deal. Revisit the guides in the settings tab',
  'onboarding_page3_title': 'Relax and let the app do the job',
  'onboarding_page3_body':
      'Follow the freelancers you want so others know you are interested and get notifyied on new deals. Add your deals and wait for someone to contact you :)',
  'onboarding_page4_title': 'Support',
  'onboarding_page4_body':
      'If some freelancers a missing or you face an issue, contact Sirvice service from settings',
  'onboarding_page5_title': 'Done',
  'onboarding_page5_body': 'Congrats, you are now ready to use the app!',
  'skip_btn_txt': 'Skip',
  'next_btn_txt': 'Next',
  'done_btn_txt': 'Done',

  // How To Page
  'how_to_page1_title': 'How to add a deal',
  'how_to_page1_body':
      'Select the freelancer you want, press the ＋ button to add a deal.',
  'how_to_page2_title': 'How to receive notifications',
  'how_to_page2_body':
      'Make sure to turn on notifications in [Settings ➞ Leaf ➞ Notifications] on iOS and [Settings ➞ Apps & notifications ➞ Leaf ➞ Notifications]. The steps may vary from android devices based on brand. If this doesent work try to delete and reinstall the app',
  'how_to_page3_title': 'How to send a fast message',
  'how_to_page3_body':
      'In the deals section you can send messages to people, Pro tip: long press the message button ➤ to send a fast message from the same page.',

  // Authentication Form
  'auth_email_hint': 'Email',
  'auth_password_hint': 'Password',
  'login_btn_text': 'Log in',
  'reg_btn_text': 'Create an account',
  'reset_pass_btn_text': 'Forgot password',

  // Create Account Bottom Sheet
  'reg_firstname_hint': 'Firstname',
  'reg_lastname_hint': 'Lastname',
  'reg_email_hint': 'Email',
  'reg_password_hint': 'Password',
  'reg_success_msg_text': 'Sent email verification, please check your inbox',

  // Reset Email Bottom Sheet
  'reset_pass_email_hint': 'Email',
  'reset_pass_send_btn_text': 'Send reset password email',
  'reset_pass_success_msg_text':
      'Reset password email sent, please check your inbox',

  // Home Page Tabs
  'freelancers_tab_text': 'Freelancers',
  'chats_tab_text': 'Chats',
  'profile_tab_text': 'Profile',
  'settings_tab_text': 'Settings',
  'follow_tab_text': 'Following',

  // Freelancers Page
  'freelancer_search_bar_text': 'Search for freelancers or authors',
  'freelancer_deals_count_text': 'Deals:',
  'freelancer_follows_count_text': 'Followings:',
  'clear_search_btn_text': 'Clear Search',

  // Deals Page
  'showcase_follow_btn_text':
      'Follow a freelancer to receive notifications on new deals',
  'showcase_filter_btn_text': 'Filter deals by price, place or quality',
  'showcase_add_btn_text': 'Add a new deal to this freelancer',
  'follow_btn_text': 'Follow',
  'follow_success_msg_text': 'Succesfully started following this freelancer',
  'clear_filter_btn_text': 'Clear Filter',
  'empty_list_msg_text': 'There are no deals yet',

  // Deal Item
  'deal_item_page_count_suffix': ' p.',

  // Add Deal Bottom Sheet
  'select_price_hint': 'Select price',
  'select_quality_hint': 'Select quality',
  'select_place_hint': 'Select place',
  'deal_description_hint':
      'Description like notes, highlighte etc.. (Optional)',
  'add_deal_btn_text': 'Add deal',
  'add_deal_success_msg_text': 'Succesfully added deal',

  // Filter Deal Bottom Sheet
  'price_from_text': 'Price from:',
  'price_to_text': 'Price to:',
  'filter_quality': 'Select quality',
  'filter_place': 'Add up to 10 places',
  'filter_deals_btn_text': 'Filter deals',

  // Follow Page
  'follow_page_topbar_title': 'Following',
  'unfollow_success_msg_text': 'Succesfully unfollowed',

  // 404 Page
  'page_not_found_text': 'Page not found',

  // Map Page
  'send_location_btn_text': 'Send Location',
  'open_in_google_maps_btn_text': 'Open in Google Maps',

  // Fast Message Bottom Sheet
  'fast_message_title': 'Send fast message to:',

  // Message Action Sheet
  'camera_action': 'Camera',
  'photo_library_action': 'Library',
  'location_action': 'Location',
  'cancel_action': 'Cancel',

  // Message Bar
  'send_message_hint_text': 'Send a message...',

  // Recipients Page
  'recipients_page_topbar_title': 'Messages',

  // Profile Page
  'member_since_text': 'Member since:',
  'edit_profile_btn_text': 'Edit profile',
  'send_message_btn_text': 'Send message',
  'deals_title_text': 'Deals',
  'profile_dealitem_delete_msg_txt': 'Succesfully deleted the deal',
  'edit_profile_image_success_msg_text': 'Successfully changed profile image',

  // Edit Profile Bottom Sheet
  'edit_profile_firstname_hint': 'Firstname',
  'edit_profile_lastname_hint': 'Lastname',
  'edit_profile_success_msg_hint': 'Succesfully edited profile',

  // Setting Page
  'setting_page_topbar_title': 'Settings',
  'settings_list_reset_pass_text': 'Reset Password',
  'settings_list_send_feedback_title': 'Send Feedback',
  'settings_list_howto_text': 'How To Guide',
  'settings_list_send_feedback_subtitle':
      'Report issues, missing freelancers or suggest improvents',
  'settings_list_log_out_text': 'Sign out',
  'settings_list_delete_account_text': 'Delete Account',
  'delete_account_btn_text': 'Delete Account',

  // Location Bottom Sheet
  'change_localization_btn_text': 'Change localization',

  // Leaf Error
  'leaf_error_msg_text': 'Something went wrong, we are working on it',
  'leaf_error_btn_text': 'Try again',

  // Global
  'error_msg': 'Something went wrong, please try again',

  // Deal Qualitites
  'New': 'New',
  'As new': 'As new',
  'Used': 'Used',
};
