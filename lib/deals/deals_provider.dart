...
import 'package:sirvice_app/deals/deals_service.dart';
import 'package:sirvice_app/deals/models/deal_filter.dart';
...
import 'package:sirvice_app/profile/profile_service.dart';
...
class DealsProvider with ChangeNotifier {
  ...
  /// for that spesific freelancer. If an error accours the stream will be canceled,
  ...
  /// After adding/updating freelancer deals, add/update the [userItems] in the users [profile],
  ...
  Future<bool> setDeal({
    ...
    try {
      ...
      // add deal to the corresponding freelancer collection database
  ...
  /// follow a [Freelancer], by subscribing to its id, so that [notifications] can
  /// be received when new deals are added. return true if successfull
  /// and false if an error occurs
  Future<bool> followFreelancer(Freelancer freelancer) async {
    ...
    try {
      ...
      final follow = Follow(
        pid: freelancer.isbn,
        title: freelancer.titles.first,
        image: freelancer.image,
        year: freelancer.year,
        time: time,
        notification: false,
      );
      await _followService.follow(uid: user.uid, follow: follow);
      await _notificationsService.subscribeToTopic(freelancer.isbn);
    } catch (error) {
      print('Add deal error: $error');
  ...
  /// certain freelancer, store result in [isFollowing] default is false i.g. no wifi
  /// if an error occurs set [isError]
  void getFollowStatus(String isbn) async {
    final user = _authenticationService.currentUser!;
    final stream = _followService.getFollowingStatus(uid: user.uid, id: isbn);
